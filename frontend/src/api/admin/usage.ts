...
export interface AdminUsageDebugDetail {
  request_id: string
  created_at: string
...
export async function getDebugDetail(requestId: string): Promise<AdminUsageDebugDetail> {
  const { data } = await apiClient.get<AdminUsageDebugDetail>(`/admin/usage/debug/${encodeURIComponent(requestId)}`)
  return data
}

export async function exportDebugDetail(requestId: string): Promise<Blob> {
  const { data } = await apiClient.get(`/admin/usage/debug/${encodeURIComponent(requestId)}/export`, {
    responseType: 'blob'
  })
  return data as Blob
}
