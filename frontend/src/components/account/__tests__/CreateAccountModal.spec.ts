...
describe('CreateAccountModal custom upstream', () => {
  it('shows TLS skip control for custom OpenAI API base URLs', async () => {
    const wrapper = mountModal()

    await switchToOpenAIAPIKey(wrapper)

    expect(wrapper.text()).not.toContain('跳过 TLS 证书校验')
    ...
    expect(wrapper.text()).not.toContain('读取上游模型')
  })
})
