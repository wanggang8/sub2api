...
const showUpstreamSettings = computed(() => (
  ['openai', 'anthropic', 'gemini', 'grok'].includes(props.account?.platform || '')
  && props.account?.type === 'apikey'
  && isCustomPlatformBaseURL(
    props.account?.platform || '',
    effectiveUpstreamBaseURL.value || getDefaultAPIKeyBaseURL(props.account?.platform || 'anthropic')
  )
))
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  editSkipTLSVerify.value = newAccount.type === 'apikey' && credentials?.skip_tls_verify === true
