      ...
      <div v-if="form.platform === 'anthropic' && accountCategory === 'bedrock'" class="space-y-4">
        ...
        <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
          ...
          <div v-else class="space-y-3">
            <div class="flex items-center justify-between gap-3 rounded-lg bg-purple-50 p-3 dark:bg-purple-900/20">
              <p class="text-xs text-purple-700 dark:text-purple-400">
                {{ t('admin.accounts.mapRequestModels') }}
              </p>
              <button
...
import { fetchModelsPreview } from '@/api/admin/accounts'
import {
  useAccountOAuth,
  type AddMethod,
...
watch(
  ...
  ([newMode]) => {
    ...
    if (supportsModelPreview.value && availableModels.value.length === 0) {
      fetchAccountModels(true)
    }
  }
)
...
const supportsModelPreview = computed(() =>
  ...
  (form.platform === 'openai' || form.platform === 'gemini' || form.platform === 'anthropic')
)

function buildModelPreviewPayload() {
...
const fetchAccountModels = async (silent = false) => {
  const previewPayload = buildModelPreviewPayload()
  ...
  try {
    const preview = await fetchModelsPreview(0, previewPayload)
    availableModels.value = preview.models.map((model) => model.id)
...
const buildOpenAIExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  delete extra.cursor_session_compat_enabled
