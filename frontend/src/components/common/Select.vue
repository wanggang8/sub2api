...
const { t } = useI18n()
const SELECT_OPEN_EVENT = 'sub2api:select-opened'
...
const handleSelectOpened = (event: Event) => {
  ...
  if (detail?.instanceId !== instanceId && isOpen.value) {
    isOpen.value = false
  }
}
...
onMounted(() => {
  ...
  window.addEventListener(SELECT_OPEN_EVENT, handleSelectOpened as EventListener)
})

onUnmounted(() => {
  document.removeEventListener('click', handleClickOutside)
  window.removeEventListener(SELECT_OPEN_EVENT, handleSelectOpened as EventListener)
  window.removeEventListener('scroll', updateTriggerRect, { capture: true })
  window.removeEventListener('resize', calculateDropdownPosition)
})
</script>
