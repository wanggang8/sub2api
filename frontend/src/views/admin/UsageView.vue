<template>
  ...
  />
  <BaseDialog
    :show="showDebugDetailModal"
    :title="t('admin.usage.debugDetail')"
    width="wide"
    @close="closeDebugDetail"
  >
    <div class="space-y-4">
      <div v-if="debugDetailLoading" class="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
        {{ t('common.loading') }}
      </div>
      <div v-else-if="!debugDetail" class="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
        {{ t('admin.usage.debugDetailEmpty') }}
      </div>
      <template v-else>
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div class="card p-4">
            <div class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">{{ t('admin.usage.requestHeaders') }}</div>
            <pre class="max-h-72 overflow-auto whitespace-pre-wrap break-all rounded bg-gray-50 p-3 text-xs dark:bg-dark-800">{{ formatDebugJSON(debugDetail.request_headers || {}) }}</pre>
          </div>
          <div class="card p-4">
            <div class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">{{ t('admin.usage.responseHeaders') }}</div>
            <pre class="max-h-72 overflow-auto whitespace-pre-wrap break-all rounded bg-gray-50 p-3 text-xs dark:bg-dark-800">{{ formatDebugJSON(debugDetail.response_headers || {}) }}</pre>
          </div>
        </div>
        <div class="card p-4">
          <div class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">{{ t('admin.usage.requestBody') }}</div>
          <div v-if="debugDetail.request_body_truncated" class="mb-2 text-xs text-amber-600 dark:text-amber-400">
            {{ t('admin.usage.debugBodyTruncated', { bytes: debugDetail.request_body_bytes || 0 }) }}
          </div>
          <pre class="max-h-80 overflow-auto whitespace-pre-wrap break-all rounded bg-gray-50 p-3 text-xs dark:bg-dark-800">{{ debugDetail.request_body || '-' }}</pre>
        </div>
        <div class="card p-4">
          <div class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">{{ t('admin.usage.responseBody') }}</div>
          <div v-if="debugDetail.response_body_truncated" class="mb-2 text-xs text-amber-600 dark:text-amber-400">
            {{ t('admin.usage.debugBodyTruncated', { bytes: debugDetail.response_body_bytes || 0 }) }}
          </div>
          <pre class="max-h-80 overflow-auto whitespace-pre-wrap break-all rounded bg-gray-50 p-3 text-xs dark:bg-dark-800">{{ debugDetail.response_body || '-' }}</pre>
        </div>
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div class="card p-4">
            <div class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">{{ t('admin.usage.upstreamRequestBody') }}</div>
            <div v-if="debugDetail.upstream_request_body_truncated" class="mb-2 text-xs text-amber-600 dark:text-amber-400">
              {{ t('admin.usage.debugBodyTruncated', { bytes: debugDetail.upstream_request_body_bytes || 0 }) }}
            </div>
            <pre class="max-h-72 overflow-auto whitespace-pre-wrap break-all rounded bg-gray-50 p-3 text-xs dark:bg-dark-800">{{ debugDetail.upstream_request_body || '-' }}</pre>
          </div>
          <div class="card p-4">
            <div class="mb-2 text-sm font-semibold text-gray-900 dark:text-white">{{ t('admin.usage.upstreamResponseBody') }}</div>
            <div v-if="debugDetail.upstream_response_body_truncated" class="mb-2 text-xs text-amber-600 dark:text-amber-400">
              {{ t('admin.usage.debugBodyTruncated', { bytes: debugDetail.upstream_response_body_bytes || 0 }) }}
            </div>
            <pre class="max-h-72 overflow-auto whitespace-pre-wrap break-all rounded bg-gray-50 p-3 text-xs dark:bg-dark-800">{{ debugDetail.upstream_response_body || '-' }}</pre>
          </div>
        </div>
      </template>
    </div>
    <template #footer>
      <div class="flex items-center justify-end gap-3">
        <button type="button" class="btn btn-secondary" @click="closeDebugDetail">
          {{ t('common.close') }}
        </button>
        <button type="button" class="btn btn-primary" :disabled="!selectedDebugRequestId" @click="exportDebugDetailFile">
          {{ t('admin.usage.exportDebugDetail') }}
        </button>
      </div>
    </template>
  </BaseDialog>
...
import type { AdminUsageLog, TrendDataPoint, ModelStat, GroupStat, EndpointStat, AdminUser } from '@/types'; import type { AdminUsageStatsResponse, AdminUsageQueryParams, AdminUsageDebugDetail } from '@/api/admin/usage'
...
const showDebugDetailModal = ref(false)
const debugDetailLoading = ref(false)
const selectedDebugRequestId = ref('')
const debugDetail = ref<AdminUsageDebugDetail | null>(null)
...
const handleDebugClick = async (requestId: string) => {
  if (!requestId) return
  selectedDebugRequestId.value = requestId
  showDebugDetailModal.value = true
  debugDetailLoading.value = true
  debugDetail.value = null
  try {
    debugDetail.value = await adminUsageAPI.getDebugDetail(requestId)
  } catch (error: any) {
    appStore.showError(error?.response?.data?.detail || error?.response?.data?.message || t('admin.usage.debugDetailLoadFailed'))
  } finally {
    debugDetailLoading.value = false
  }
}

const closeDebugDetail = () => {
  showDebugDetailModal.value = false
  debugDetailLoading.value = false
  selectedDebugRequestId.value = ''
  debugDetail.value = null
}

const exportDebugDetailFile = async () => {
  if (!selectedDebugRequestId.value) return
  try {
    const blob = await adminUsageAPI.exportDebugDetail(selectedDebugRequestId.value)
    saveAs(blob, `usage-debug-${selectedDebugRequestId.value}.json`)
  } catch (error: any) {
    appStore.showError(error?.response?.data?.detail || error?.response?.data?.message || t('admin.usage.debugDetailExportFailed'))
  }
}
...
const allColumns = computed(() => [
  ...
  { key: 'debug', label: t('admin.usage.debugDetail'), sortable: false },
