...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	usageDebugService := service.ProvideUsageDebugService(redisClient)
	...
	adminUsageHandler := admin.NewUsageHandler(usageService, apiKeyService, adminService, usageCleanupService, usageDebugService)
	...
	gatewayHandler := handler.NewGatewayHandler(gatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, usageDebugService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, userMessageQueueService, configConfig, settingService)
	openAIGatewayHandler := handler.NewOpenAIGatewayHandler(openAIGatewayService, concurrencyService, billingCacheService, apiKeyService, usageDebugService, usageRecordWorkerPool, errorPassthroughService, configConfig)
	cursorCompatHandler := handler.NewCursorCompatHandler(gatewayHandler, openAIGatewayHandler)
	augmentCompatHandler := handler.NewAugmentCompatHandler(gatewayHandler, openAIGatewayHandler)
	handlerSettingHandler := handler.ProvideSettingHandler(settingService, buildInfo)
	totpHandler := handler.NewTotpHandler(totpService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, cursorCompatHandler, augmentCompatHandler, handlerSettingHandler, totpHandler, idempotencyCoordinator, idempotencyCleanupService)
