...
// NewAPIKeyAuthMiddlewareWithWriter 创建带自定义错误输出的 API Key 鉴权中间件。
func NewAPIKeyAuthMiddlewareWithWriter(
	apiKeyService *service.APIKeyService,
	subscriptionService *service.SubscriptionService,
	cfg *config.Config,
	errorWriter APIKeyAuthErrorWriter,
...
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config, errorWriter APIKeyAuthErrorWriter) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		abortWithQuotaError := func() {
			if errorWriter != nil {
				errorWriter(c, http.StatusTooManyRequests, "API_KEY_QUOTA_EXHAUSTED", "API key 额度已用完")
				return
			}
		...
		if rejectInvalidAuthAbuse(c, apiKeyService) {
			abortWithError(http.StatusTooManyRequests, "INVALID_AUTH_RATE_LIMITED", "Too many invalid authentication attempts; retry later")
			return
		}
		...
		if apiKeyHeadersTooLarge(c) {
			...
			abortWithError(http.StatusUnauthorized, "INVALID_API_KEY", "Invalid API key")
			return
		}
		...
		if err != nil {
			...
			if errors.Is(err, service.ErrAPIKeyAuthOverloaded) {
				...
				abortWithError(http.StatusServiceUnavailable, "API_KEY_AUTH_OVERLOADED", "API key authentication is temporarily unavailable")
				return
			}
