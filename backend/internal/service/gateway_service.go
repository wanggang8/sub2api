...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		// 发送请求
		resp, err := s.httpUpstream.DoWithTLS(ApplyAccountUpstreamRequestOptions(upstreamReq, account), proxyURL, account.ID, account.Concurrency, tlsProfile)
		if err != nil {
		...
		if resp.StatusCode == 400 {
			...
			if readErr == nil {
				...
				if s.shouldRectifySignatureError(ctx, account, respBody) {
					...
					releaseRetryCtx()
					if buildErr == nil {
						retryResp, retryErr := s.httpUpstream.DoWithTLS(ApplyAccountUpstreamRequestOptions(retryReq, account), proxyURL, account.ID, account.Concurrency, tlsProfile)
						if retryErr == nil {
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isSignatureErrorPattern(ctx, account, retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									releaseRetryCtx2()
									if buildErr2 == nil {
										retryResp2, retryErr2 := s.httpUpstream.DoWithTLS(ApplyAccountUpstreamRequestOptions(retryReq2, account), proxyURL, account.ID, account.Concurrency, tlsProfile)
										if retryErr2 == nil {
											resp = retryResp2
											break
										}
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						if buildErr == nil {
							budgetRetryResp, retryErr := s.httpUpstream.DoWithTLS(ApplyAccountUpstreamRequestOptions(budgetRetryReq, account), proxyURL, account.ID, account.Concurrency, tlsProfile)
							if retryErr == nil {
...
func (s *GatewayService) forwardAnthropicAPIKeyPassthroughWithInput(
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		resp, err = s.httpUpstream.DoWithTLS(ApplyAccountUpstreamRequestOptions(upstreamReq, account), proxyURL, account.ID, account.Concurrency, s.tlsFPProfileService.ResolveTLSProfile(account))
		if err != nil {
			if resp != nil && resp.Body != nil {
				_ = resp.Body.Close()
			}
...
func (s *GatewayService) executeBedrockUpstream(
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		resp, err = s.httpUpstream.DoWithTLS(ApplyAccountUpstreamRequestOptions(upstreamReq, account), proxyURL, account.ID, account.Concurrency, nil)
		if err != nil {
			if resp != nil && resp.Body != nil {
				_ = resp.Body.Close()
			}
...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	if resp.StatusCode == 400 && s.shouldRectifySignatureError(ctx, account, respBody) {
		...
		if buildErr == nil {
			retryResp, retryErr := s.httpUpstream.DoWithTLS(ApplyAccountUpstreamRequestOptions(retryReq, account), proxyURL, account.ID, account.Concurrency, s.tlsFPProfileService.ResolveTLSProfile(account))
			if retryErr == nil {
				resp = retryResp
				respBody, err = ReadUpstreamResponseBody(resp.Body, s.cfg, c, countTokensTooLarge)
				_ = resp.Body.Close()
