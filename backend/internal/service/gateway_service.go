...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		// 发送请求
		resp, err = s.httpUpstream.DoWithTLS(upstreamReq, proxyURL, account.ID, account.Concurrency, UpstreamTLSOptionsFromAccount(account, tlsProfile))
		if err != nil {
		...
		if resp.StatusCode == 400 {
			...
			if readErr == nil {
				...
				if s.shouldRectifySignatureError(ctx, account, respBody) {
					...
					releaseRetryCtx()
					if buildErr == nil {
						retryResp, retryErr := s.httpUpstream.DoWithTLS(retryReq, proxyURL, account.ID, account.Concurrency, UpstreamTLSOptionsFromAccount(account, tlsProfile))
						if retryErr == nil {
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isSignatureErrorPattern(ctx, account, retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									releaseRetryCtx2()
									if buildErr2 == nil {
										retryResp2, retryErr2 := s.httpUpstream.DoWithTLS(retryReq2, proxyURL, account.ID, account.Concurrency, UpstreamTLSOptionsFromAccount(account, tlsProfile))
										if retryErr2 == nil {
											resp = retryResp2
											break
										}
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						if buildErr == nil {
							budgetRetryResp, retryErr := s.httpUpstream.DoWithTLS(budgetRetryReq, proxyURL, account.ID, account.Concurrency, UpstreamTLSOptionsFromAccount(account, tlsProfile))
							if retryErr == nil {
...
func (s *GatewayService) handleStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string, mimicClaudeCode bool) (*streamingResult, error) {
	...
	processSSEEvent := func(lines []string) ([]string, string, *sseUsagePatch, error) {
		...
		upstreamCapture.Write([]byte(strings.Join(lines, "\n")))
...
type RecordUsageInput struct {
	...
	ResolvedRequestID  string             // 已统一解析好的 usage/debug request_id，优先用于持久化关联
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	// 计算费用。若请求模型只是别名且无定价，则回退到上游实际模型计费。
	cost, billingModel := s.calculateRecordUsageCost(ctx, result, apiKey, billingModel, multiplier, opts)
...
func (s *GatewayService) calculateRecordUsageCost(
	...
	if result.ImageCount > 0 {
		cost, err := s.calculateImageCost(ctx, result, apiKey, billingModel, multiplier)
		if err == nil || strings.TrimSpace(result.UpstreamModel) == "" || strings.EqualFold(strings.TrimSpace(billingModel), strings.TrimSpace(result.UpstreamModel)) || !isPricingLookupMiss(err) {
			return cost, billingModel
		}
		fallbackCost, fallbackErr := s.calculateImageCost(ctx, result, apiKey, result.UpstreamModel, multiplier)
		if fallbackErr == nil {
			return fallbackCost, strings.TrimSpace(result.UpstreamModel)
		}
		return cost, billingModel
	}

	// Token 计费
	cost, resolvedModel, err := calculateUsageCostWithUpstreamFallback(billingModel, result.UpstreamModel, func(model string) (*CostBreakdown, error) {
		return s.calculateTokenCost(ctx, result, apiKey, model, multiplier, opts)
	})
	if err != nil {
		logger.LegacyPrintf("service.gateway", "Calculate cost failed: %v", err)
		return &CostBreakdown{ActualCost: 0}, billingModel
	}
...
func (s *GatewayService) buildRecordUsageLog(
	...
	requestID := strings.TrimSpace(input.ResolvedRequestID)
	if requestID == "" {
...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	if resp.StatusCode == 400 && s.shouldRectifySignatureError(ctx, account, respBody) {
		...
		if buildErr == nil {
			retryResp, retryErr := s.httpUpstream.DoWithTLS(retryReq, proxyURL, account.ID, account.Concurrency, UpstreamTLSOptionsFromAccount(account, s.tlsFPProfileService.ResolveTLSProfile(account)))
			if retryErr == nil {
				resp = retryResp
				respBody, err = readUpstreamResponseBodyLimited(resp.Body, maxReadBytes)
				_ = resp.Body.Close()
