...
func TestOpenAIGatewayServiceRecordUsage_UsesExplicitResolvedRequestID(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
	...
	err := svc.RecordUsage(ctx, &OpenAIRecordUsageInput{
		...
		User:              &User{ID: 20049},
		Account:           &Account{ID: 30049},
		ResolvedRequestID: "debug:openai-fixed-1",
	})

	require.NoError(t, err)
	require.NotNil(t, billingRepo.lastCmd)
	require.Equal(t, "debug:openai-fixed-1", billingRepo.lastCmd.RequestID)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "debug:openai-fixed-1", usageRepo.lastLog.RequestID)
}
...
func TestOpenAIGatewayServiceRecordUsage_FallsBackToUpstreamModelWhenRequestedPricingMissing(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
	svc := newOpenAIRecordUsageServiceForTest(usageRepo, userRepo, subRepo, nil)
	...
	err = svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:     "resp_requested_pricing_fallback",
			Model:         "CustomModel",
	...
	require.NoError(t, err)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "CustomModel", usageRepo.lastLog.Model)
	require.NotNil(t, usageRepo.lastLog.UpstreamModel)
