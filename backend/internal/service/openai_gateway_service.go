...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	for {
		...
		// Send request
		setOpsUpstreamRequestBody(c, readAndRestoreRequestBody(upstreamReq))
		upstreamStart := time.Now()
		resp, err := s.httpUpstream.Do(ApplyAccountUpstreamRequestOptions(upstreamReq, account), proxyURL, account.ID, account.Concurrency)
...
func openAIUsageFromResult(usageNode gjson.Result) OpenAIUsage {
	return OpenAIUsage{
		...
		ImageOutputTokens:    openAIUsageInt(usageNode, "output_tokens_details.image_tokens", "completion_tokens_details.image_tokens"),
	}
}
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if isOpenAIForwardUsageEmpty(result) && result.ImageCount == 0 && (apiKey == nil || apiKey.Group == nil) {
		return nil
	}
	...
	hasChannelMappedBillingModel := input.BillingModelSource == BillingModelSourceChannelMapped &&
		input.ChannelMappedModel != "" &&
		input.ChannelMappedModel != input.OriginalModel
	if hasChannelMappedBillingModel {
...
func isOpenAIForwardUsageEmpty(result *OpenAIForwardResult) bool {
	if result == nil {
		return true
	}
