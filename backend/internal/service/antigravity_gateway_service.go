...
func (s *AntigravityGatewayService) handleSmartRetry(p antigravityRetryLoopParams, resp *http.Response, respBody []byte, baseURL string, urlIdx int, availableURLs []string) *smartRetryResult {
	...
	if shouldSmartRetry {
		...
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			...
			retryResp, retryErr := p.httpUpstream.DoWithTLS(retryReq, p.proxyURL, p.account.ID, p.account.Concurrency, UpstreamTLSOptionsFromAccount(p.account, nil))
...
urlFallbackLoop:
	for urlIdx, baseURL := range availableURLs {
		...
		for attempt := 1; attempt <= antigravityMaxRetries; attempt++ {
			...
			resp, err = p.httpUpstream.DoWithTLS(upstreamReq, p.proxyURL, p.account.ID, p.account.Concurrency, UpstreamTLSOptionsFromAccount(p.account, nil))
			if err == nil && resp == nil {
		...
		if s.settingService != nil && s.settingService.IsModelFallbackEnabled(ctx) &&
			...
			if fallbackModel != "" && fallbackModel != mappedModel {
				...
				if err == nil {
					...
					if err == nil {
						fallbackResp, err := s.httpUpstream.DoWithTLS(fallbackReq, proxyURL, account.ID, account.Concurrency, UpstreamTLSOptionsFromAccount(account, nil))
