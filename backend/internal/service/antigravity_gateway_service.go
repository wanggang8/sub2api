...
func (s *AntigravityGatewayService) handleSmartRetry(p antigravityRetryLoopParams, resp *http.Response, respBody []byte, baseURL string, urlIdx int, availableURLs []string) *smartRetryResult {
	...
	if shouldSmartRetry {
		...
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			...
			retryResp, retryErr := p.httpUpstream.Do(ApplyAccountUpstreamRequestOptions(retryReq, p.account), p.proxyURL, p.account.ID, p.account.Concurrency)
...
urlFallbackLoop:
	for urlIdx, baseURL := range availableURLs {
		...
		for attempt := 1; attempt <= antigravityMaxRetries; attempt++ {
			...
			resp, err = p.httpUpstream.Do(ApplyAccountUpstreamRequestOptions(upstreamReq, p.account), p.proxyURL, p.account.ID, p.account.Concurrency)
			if err == nil && resp == nil {
		...
		if s.settingService != nil && s.settingService.IsModelFallbackEnabled(ctx) &&
			...
			if fallbackModel != "" && fallbackModel != mappedModel {
				...
				if err == nil {
					...
					if err == nil {
						fallbackResp, err := s.httpUpstream.Do(ApplyAccountUpstreamRequestOptions(fallbackReq, account), proxyURL, account.ID, account.Concurrency)
