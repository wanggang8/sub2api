...
func TestOpenAIGatewayService_Forward_WSv2_UsesForcedCodexInstructionsAfterCachedDecode(t *testing.T) {
	gin.SetMode(gin.TestMode)

	receivedInstructions := make(chan string, 1)
	upgrader := websocket.Upgrader{CheckOrigin: func(r *http.Request) bool { return true }}
	wsServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		conn, err := upgrader.Upgrade(w, r, nil)
		if err != nil {
		...
		receivedInstructions <- strings.TrimSpace(firstNonEmptyString(request["instructions"]))

		if err := conn.WriteJSON(map[string]any{
			"type": "response.completed",
			"response": map[string]any{
				"id":    "resp_forced_instructions_ws_1",
				"model": "gpt-5.4",
				"usage": map[string]any{"input_tokens": 1, "output_tokens": 1},
			},
		}); err != nil {
	...
	account := &Account{
		ID:          11,
		Name:        "openai-ws-forced-instructions",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	// The image_generation declaration forces an early full-map decode. The
	// forced template must invalidate that cache before the WS payload is built.
	body := []byte(`{"model":"gpt-5.4","stream":false,"instructions":"client instructions","tools":[{"type":"image_generation"}],"input":[{"type":"input_text","text":"hello"}]}`)
	result, err := svc.Forward(context.Background(), c, account, body)
	require.NoError(t, err)
	require.NotNil(t, result)
	require.True(t, result.OpenAIWSMode)
	require.Equal(t, "server-prefix\n\nclient instructions", <-receivedInstructions)
}
