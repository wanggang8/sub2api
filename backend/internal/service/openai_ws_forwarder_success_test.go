...
func TestOpenAIGatewayService_Forward_WSv2KeepsCursorToolCallsOnCompletedEvent(t *testing.T) {
	gin.SetMode(gin.TestMode)

	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/cursor/v1/responses", nil)
	c.Request.Header.Set("User-Agent", "cursor")
	...
	captureConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.completed","response":{"id":"resp_cursor_tool_1","model":"gpt-5.1","tool_calls":[{"function":{"name":"apply_patch","arguments":"{}"}}],"usage":{"input_tokens":2,"output_tokens":1}},"tool_calls":[{"function":{"name":"apply_patch","arguments":"{}"}}]}`),
		},
	}
	captureDialer := &openAIWSCaptureDialer{conn: captureConn}
	pool := newOpenAIWSConnPool(cfg)
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Equal(t, "resp_cursor_tool_1", result.RequestID)
	require.Equal(t, "apply_patch", gjson.GetBytes(rec.Body.Bytes(), "tool_calls.0.function.name").String())
