...
func TestGatewayServiceRecordUsage_FallsBackToUpstreamModelWhenRequestedPricingMissing(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
	svc := newGatewayRecordUsageServiceForTest(usageRepo, userRepo, subRepo)

	expectedCost, err := svc.billingService.CalculateCost("claude-sonnet-4", UsageTokens{
		InputTokens:  10,
		OutputTokens: 6,
	}, 1.1)
	require.NoError(t, err)
	...
	err = svc.RecordUsage(context.Background(), &RecordUsageInput{
		Result: &ForwardResult{
			RequestID:     "gateway_requested_pricing_fallback",
			Usage:         ClaudeUsage{InputTokens: 10, OutputTokens: 6},
			Model:         "CustomModel",
	...
	require.True(t, usageRepo.lastLog.ActualCost > 0, "fallback pricing should avoid zero-cost usage logs")
}
...
func TestGatewayServiceRecordUsage_UsesExplicitResolvedRequestID(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	svc := newGatewayRecordUsageServiceWithBillingRepoForTest(usageRepo, billingRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{})
	...
	err := svc.RecordUsage(ctx, &RecordUsageInput{
		...
		User:              &User{ID: 606},
		Account:           &Account{ID: 706},
		ResolvedRequestID: "debug:anthropic-fixed-1",
	})

	require.NoError(t, err)
	require.NotNil(t, billingRepo.lastCmd)
	require.Equal(t, "debug:anthropic-fixed-1", billingRepo.lastCmd.RequestID)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "debug:anthropic-fixed-1", usageRepo.lastLog.RequestID)
}
