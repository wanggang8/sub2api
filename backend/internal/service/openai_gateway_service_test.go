...
func TestOpenAIGatewayService_ResolveCursorCompatPromptCacheKey(t *testing.T) {
	gin.SetMode(gin.TestMode)

	t.Run("returns existing session header when enabled on cursor route", func(t *testing.T) {
		rec := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(rec)
		c.Request = httptest.NewRequest(http.MethodPost, "/cursor/v1/responses", nil)
		c.Request.Header.Set("session_id", "sess-existing")

		svc := &OpenAIGatewayService{}
		got := svc.ResolveCursorCompatPromptCacheKey(c, []byte(`{"model":"gpt-5.4"}`))
		require.Equal(t, "sess-existing", got)
	})

	t.Run("generates deterministic prompt cache key when missing on cursor route", func(t *testing.T) {
		rec := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(rec)
		c.Request = httptest.NewRequest(http.MethodPost, "/cursor/v1/responses", nil)
		...
		got := svc.ResolveCursorCompatPromptCacheKey(c, body)
		require.Equal(t, GenerateSessionUUID(deriveOpenAIContentSessionSeed(body)), got)
	})
	...
	t.Run("keeps same key when later turns only append history", func(t *testing.T) {
		...
		round2 := []byte(`{
			...
			"messages":[
				...
				{"role":"user","content":"继续修复这个 bug"}
			],
			"tools":[{"type":"function","function":{"name":"Read","description":"read file"}}]
		}`)

		key1 := svc.ResolveCursorCompatPromptCacheKey(c, round1)
		key2 := svc.ResolveCursorCompatPromptCacheKey(c, round2)
		require.NotEmpty(t, key1)
		require.Equal(t, key1, key2, "later turns should preserve the same compat key")
	})

	t.Run("different first user produces different key even with same system and tools", func(t *testing.T) {
		rec := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(rec)
		c.Request = httptest.NewRequest(http.MethodPost, "/cursor/v1/chat/completions", nil)
		...
		bodyB := []byte(`{
			...
			"messages":[
				{"role":"system","content":"You are an AI coding assistant."},
				{"role":"user","content":"帮我写一个新功能"}
			],
			"tools":[{"type":"function","function":{"name":"Read","description":"read file"}}]
		}`)

		keyA := svc.ResolveCursorCompatPromptCacheKey(c, bodyA)
		keyB := svc.ResolveCursorCompatPromptCacheKey(c, bodyB)
		require.NotEmpty(t, keyA)
		require.NotEmpty(t, keyB)
		require.NotEqual(t, keyA, keyB, "first user message should split concurrent windows into different buckets")
	})
...
func TestInjectOpenAIPromptCacheKeyIfMissing(t *testing.T) {
	body := []byte(`{"model":"gpt-5.4","input":"hello"}`)
	key := "sess-injected"

	patched := InjectOpenAIPromptCacheKeyIfMissing(body, key)
	require.Equal(t, key, gjson.GetBytes(patched, "prompt_cache_key").String())

	existing := []byte(`{"model":"gpt-5.4","prompt_cache_key":"keep-me","input":"hello"}`)
	unchanged := InjectOpenAIPromptCacheKeyIfMissing(existing, key)
	require.Equal(t, "keep-me", gjson.GetBytes(unchanged, "prompt_cache_key").String())
}
...
func TestOpenAIStreamingPassthroughChatCompletionsFinalChunkExtractsUsage(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize: defaultMaxLineSize,
	...
	go func() {
		...
		_, _ = pw.Write([]byte("data: {\"id\":\"chatcmpl-1\",\"object\":\"chat.completion.chunk\",\"choices\":[],\"usage\":{\"prompt_tokens\":12,\"completion_tokens\":6,\"prompt_tokens_details\":{\"cached_tokens\":2}}}\n\n"))
...
func TestOpenAIStreamingStoresUpstreamResponseBodyOnContext(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
	...
	raw, ok := c.Get(OpsUpstreamResponseBodyKey)
	require.True(t, ok)
	require.Contains(t, string(raw.([]byte)), `"response.completed"`)
}

func TestOpenAIStreamingTerminalReadErrorStillStoresUpstreamResponseMetadata(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
	...
	resp := &http.Response{
		...
		Body: &scriptedReadCloser{
			...
			finalErr: errors.New("upstream read failed after terminal event"),
		},
		Header: http.Header{},
	}
	...
	bytesLen, ok := c.Get(OpsUpstreamResponseBodyBytesKey)
	require.True(t, ok)
	require.GreaterOrEqual(t, bytesLen.(int), len(raw.([]byte)))

	truncated, ok := c.Get(OpsUpstreamResponseBodyTruncatedKey)
	require.False(t, ok)
	require.Nil(t, truncated)
}

func TestOpenAIMessagesStreamingStoresUpstreamResponseBodyOnContext(t *testing.T) {
	gin.SetMode(gin.TestMode)
	svc := &OpenAIGatewayService{}
	...
	_, err := svc.handleAnthropicStreamingResponse(resp, c, "model", "billing-model", "upstream-model", time.Now())
	require.NoError(t, err)
...
func TestOpenAIMessagesStreamingClientDisconnectStillStoresUpstreamResponseMetadata(t *testing.T) {
	gin.SetMode(gin.TestMode)
	svc := &OpenAIGatewayService{}
...
func TestOpenAIMessagesBufferedStreamingStoresUpstreamResponseBodyOnContext(t *testing.T) {
	gin.SetMode(gin.TestMode)
	svc := &OpenAIGatewayService{}
	...
	go func() {
		defer func() { _ = pw.Close() }()
		_, _ = pw.Write([]byte("data: {\"type\":\"response.completed\",\"response\":{\"output\":[],\"usage\":{\"input_tokens\":1,\"output_tokens\":2}}}\n\n"))
	}()

	_, err := svc.handleAnthropicBufferedStreamingResponse(resp, c, "model", "billing-model", "upstream-model", time.Now())
	require.NoError(t, err)
...
func TestOpenAIBuildUpstreamRequestOpenAIPassthroughPreservesCompactPath(t *testing.T) {
	...
	req, err := svc.buildUpstreamRequestOpenAIPassthrough(c.Request.Context(), c, account, []byte(`{"model":"gpt-5"}`), "token", OpenAIUpstreamAPIResponses)
	require.NoError(t, err)
...
func TestOpenAIBuildUpstreamRequestPreservesInboundUserAgentByDefault(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader([]byte(`{"model":"gpt-5"}`)))
	c.Request.Header.Set("User-Agent", "Cursor/1.2.3")
	...
	req, err := svc.buildUpstreamRequest(c.Request.Context(), c, account, []byte(`{"model":"gpt-5"}`), "token", false, "", false)
	require.NoError(t, err)
	require.Equal(t, "Cursor/1.2.3", req.Header.Get("User-Agent"))
}
...
func TestParseSSEUsage_SelectiveParsing(t *testing.T) {
	...
	// 直连 chat.completions 的最终 chunk 可能在顶层 usage 里返回 prompt/completion tokens
	svc.parseSSEUsage(`{"id":"chatcmpl-1","object":"chat.completion.chunk","choices":[],"usage":{"prompt_tokens":21,"completion_tokens":34,"prompt_tokens_details":{"cached_tokens":5}}}`, usage)
	require.Equal(t, 21, usage.InputTokens)
	require.Equal(t, 34, usage.OutputTokens)
	require.Equal(t, 5, usage.CacheReadInputTokens)
}

func TestExtractOpenAIUsageFromJSONBytes_SupportsChatCompletionsUsage(t *testing.T) {
	usage, ok := extractOpenAIUsageFromJSONBytes([]byte(`{"id":"chatcmpl-1","object":"chat.completion","usage":{"prompt_tokens":11,"completion_tokens":17,"prompt_tokens_details":{"cached_tokens":3}}}`))
	require.True(t, ok)
