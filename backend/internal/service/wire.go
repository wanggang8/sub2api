...
// ProvideOAuthRefreshAPI wires OAuthRefreshAPI without variadic lock TTL (Wire cannot inject ...time.Duration).
...
func ProvideUsageDebugService(redisClient *redis.Client) *UsageDebugService {
	return NewUsageDebugService(redisClient)
}
