...
func TestSetOpsUpstreamResponseBodyStoredOnContext(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)

	setOpsUpstreamResponseBody(c, []byte(`{"id":"resp_1"}`))

	raw, ok := c.Get(OpsUpstreamResponseBodyKey)
	require.True(t, ok)
	require.Equal(t, []byte(`{"id":"resp_1"}`), raw)
}

func TestSetOpsUpstreamResponseBodyWithMeta_StoresBytesAndTruncation(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)

	setOpsUpstreamResponseBodyWithMeta(c, []byte(`{"id":"resp_1"}`), 4096, true)
