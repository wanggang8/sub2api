...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if !s.isAccountTransportCompatible(account, req.RequiredTransport) || !s.isAccountRequestCompatible(account, req) {
		_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		return nil, nil
	}
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatible(account *Account, req OpenAIAccountScheduleRequest) bool {
	...
	return account.SupportsOpenAIImageCapability(req.RequiredImageCapability) &&
		s.isAccountCapabilityCompatible(account, req.RequiredCapability)
}
...
func (s *OpenAIGatewayService) SelectAccountWithScheduler(
	...
	upstreamCapability := OpenAIUpstreamCapabilityAny
	if len(requiredCapability) > 0 {
		upstreamCapability = requiredCapability[0]
	}
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, "", upstreamCapability)
}
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImages(
	...
	selection, decision, err := s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, requiredCapability, OpenAIUpstreamCapabilityAny)
	if err == nil && selection != nil && selection.Account != nil {
		return selection, decision, nil
	}
	// 如果要求 native 能力（如指定了模型）但没有可用的 APIKey 账号，回退到 basic（OAuth 账号）
	if requiredCapability == OpenAIImagesCapabilityNative {
		return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, OpenAIImagesCapabilityBasic, OpenAIUpstreamCapabilityAny)
	}
	...
	if scheduler == nil {
		...
		matchesRequiredCapabilities := func(account *Account) bool {
			return account != nil &&
				...
				openAIAccountCapabilityCompatible(account, requiredCapability)
		}
		if requiredTransport == OpenAIUpstreamTransportAny || requiredTransport == OpenAIUpstreamTransportHTTPSSE {
