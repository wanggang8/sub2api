...
const (
	...
	// Best-effort capture of the raw upstream response body for successful
	// requests so admin usage drilldown can compare upstream vs client-facing
	// payloads after local rewrites.
	OpsUpstreamResponseBodyKey = "ops_upstream_response_body"
	OpsUpstreamResponseBodyBytesKey = "ops_upstream_response_body_bytes"
	OpsUpstreamResponseBodyTruncatedKey = "ops_upstream_response_body_truncated"
...
func setOpsUpstreamResponseBody(c *gin.Context, body []byte) {
	setOpsUpstreamResponseBodyWithMeta(c, body, len(body), false)
}

func setOpsUpstreamResponseBodyWithMeta(c *gin.Context, body []byte, bytesLen int, truncated bool) {
	if c == nil || len(body) == 0 {
		return
	}
	c.Set(OpsUpstreamResponseBodyKey, body)
	if bytesLen > 0 {
		c.Set(OpsUpstreamResponseBodyBytesKey, bytesLen)
	}
	if truncated {
		c.Set(OpsUpstreamResponseBodyTruncatedKey, true)
	}
}
