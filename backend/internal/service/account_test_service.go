...
func (s *AccountTestService) testBedrockAccountConnection(c *gin.Context, ctx context.Context, account *Account, testModelID string) error {
	...
	resp, err := s.httpUpstream.DoWithTLS(ApplyAccountUpstreamRequestOptions(req, account), proxyURL, account.ID, account.Concurrency, nil)
	if err != nil {
		return s.sendErrorAndEnd(c, fmt.Sprintf("Request failed: %s", err.Error()))
	}
	defer func() { _ = resp.Body.Close() }()
...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	payload, err := createOpenAIAccountTestPayload(testModelID, isOAuth, protocol, prompt)
	if err != nil {
...
func createOpenAIAccountTestPayload(modelID string, isOAuth bool, protocol openAIAccountTestProtocol, prompt string) (map[string]any, error) {
	switch protocol {
	case openAIAccountTestProtocolMessages:
		return createTestPayload(modelID)
	case openAIAccountTestProtocolChatCompletions:
		return createOpenAIChatCompletionsTestPayload(modelID, prompt), nil
	default:
...
func (s *AccountTestService) testOpenAIImageOAuth(c *gin.Context, ctx context.Context, account *Account, modelID, prompt string) error {
	...
	resp, err := s.httpUpstream.Do(ApplyAccountUpstreamRequestOptions(req, account), proxyURL, account.ID, account.Concurrency)
	if err != nil {
