...
type gatewayScriptedReadCloser struct {
	chunks   [][]byte
	finalErr error
}

func (r *gatewayScriptedReadCloser) Read(p []byte) (int, error) {
	for len(r.chunks) > 0 {
		chunk := r.chunks[0]
		if len(chunk) == 0 {
			r.chunks = r.chunks[1:]
...
func (r *gatewayScriptedReadCloser) Close() error { return nil }
...
func TestHandleStreamingResponse_TerminalReadErrorStillStoresUpstreamResponseMetadata(t *testing.T) {
	gin.SetMode(gin.TestMode)
	svc := newMinimalGatewayService()
	...
	resp := &http.Response{
		...
		Header:     http.Header{},
		Body: &gatewayScriptedReadCloser{
			chunks: [][]byte{
				[]byte("data: [DONE]\n\n"),
			},
	...
	require.True(t, ok)
	require.Contains(t, string(raw.([]byte)), "[DONE]")
