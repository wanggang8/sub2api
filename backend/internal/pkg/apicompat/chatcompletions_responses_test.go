...
func TestChatCompletionsChunkToResponsesEvents_ContentAndToolCallSameChunk(t *testing.T) {
	...
	assert.Equal(t, content, events[3].Delta)
	assert.Equal(t, "response.output_item.added", events[4].Type)
	require.NotNil(t, events[4].Item)
	assert.Equal(t, "function_call", events[4].Item.Type)
	assert.Equal(t, "call_1", events[4].Item.CallID)
	assert.Equal(t, "read_file", events[4].Item.Name)
	assert.Equal(t, "response.function_call_arguments.delta", events[5].Type)
	assert.Equal(t, `{"path":"README.md"}`, events[5].Delta)

	finalized := FinalizeChatCompletionsResponsesStream(state)
	require.Len(t, finalized, 6)
	assert.Equal(t, "response.output_text.done", finalized[0].Type)
	assert.Equal(t, content, finalized[0].Text)
	assert.Equal(t, "response.content_part.done", finalized[1].Type)
	assert.Equal(t, "response.output_item.done", finalized[2].Type)
	require.NotNil(t, finalized[2].Item)
	assert.Equal(t, "message", finalized[2].Item.Type)
	assert.Equal(t, "response.function_call_arguments.done", finalized[3].Type)
	assert.Equal(t, `{"path":"README.md"}`, finalized[3].Arguments)
	assert.Equal(t, "response.output_item.done", finalized[4].Type)
	require.NotNil(t, finalized[4].Item)
	assert.Equal(t, "function_call", finalized[4].Item.Type)
	assert.Equal(t, "call_1", finalized[4].Item.CallID)
	assert.Equal(t, `{"path":"README.md"}`, finalized[4].Item.Arguments)
	assert.Equal(t, events[4].Item.ID, finalized[4].Item.ID)
	assert.Equal(t, "response.completed", finalized[5].Type)
	require.NotNil(t, finalized[5].Response)
	require.Len(t, finalized[5].Response.Output, 2)
	assert.Equal(t, "message", finalized[5].Response.Output[0].Type)
	assert.Equal(t, "function_call", finalized[5].Response.Output[1].Type)
	assert.Equal(t, "call_1", finalized[5].Response.Output[1].CallID)
	assert.Equal(t, `{"path":"README.md"}`, finalized[5].Response.Output[1].Arguments)
	assert.Equal(t, events[4].Item.ID, finalized[5].Response.Output[1].ID)
}
