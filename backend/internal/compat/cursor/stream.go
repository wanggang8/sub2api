...
func flushMessagesPendingItems(state *MessagesStreamState) []messagesSSEItem {
	...
	for _, item := range state.pending {
		results = append(results, messagesSSEItem{eventName: item.eventName, payload: shiftMessagesIndex(item.payload, state.indexOffset)})
	}
	state.pending = nil
	return results
}
...
func emitMessagesThinking(text string) []messagesSSEItem {
	...
	return []messagesSSEItem{
		{eventName: "content_block_start", payload: map[string]any{"type": "content_block_start", "index": 0, "content_block": map[string]any{"type": "thinking", "thinking": ""}}},
		{eventName: "content_block_delta", payload: map[string]any{"type": "content_block_delta", "index": 0, "delta": map[string]any{"type": "thinking_delta", "thinking": text}}},
		{eventName: "content_block_stop", payload: map[string]any{"type": "content_block_stop", "index": 0}},
	}
}
