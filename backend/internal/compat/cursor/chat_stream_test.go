...
func TestPatchChatStreamChunkConvertsLegacyFunctionCallDelta(t *testing.T) {
	state := NewChatStreamState()
	bundle := []byte(`data: {"id":"cmpl_legacy","object":"chat.completion.chunk","model":"upstream","choices":[{"index":0,"delta":{"function_call":{"name":"read_file","arguments":"{\"path\":\"README.md\"}"}},"finish_reason":"function_call"}]}` + "\n\n")
	...
	choice := payloadChoice(t, payloads[0])
