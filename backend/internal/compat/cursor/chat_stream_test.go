...
func TestPatchChatStreamChunkPreservesEmptyToolCallArgumentsForStartedCall(t *testing.T) {
	state := NewChatStreamState()
...
func TestPatchChatStreamChunkUnwrapsApplyPatchArgumentsForCursorCustomTool(t *testing.T) {
	state := NewChatStreamState()
	patch := "*** Begin Patch\n*** Update File: docs/a.md\n@@\n-old\n+new\n*** End Patch"

	start, err := PatchChatStreamChunk([]byte(`data: {"id":"cmpl_1","object":"chat.completion.chunk","model":"upstream","choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"id":"call_1","type":"function","function":{"name":"ApplyPatch","arguments":""}}]},"finish_reason":null}]}`+"\n\n"), "cursor-model", state)
	require.NoError(t, err)
	require.Contains(t, string(start), `"arguments":""`)

	wrappedArguments, err := json.Marshal(map[string]string{"patch": patch})
	require.NoError(t, err)
	argsChunk, err := json.Marshal(map[string]any{
		"id":     "cmpl_1",
					...
					"tool_calls": []any{
						map[string]any{
							...
							"function": map[string]any{
								"arguments": string(wrappedArguments),
							},
						},
					},
				},
	...
	buffered, err := PatchChatStreamChunk(append([]byte("data: "), append(argsChunk, []byte("\n\n")...)...), "cursor-model", state)
	require.NoError(t, err)
	require.Empty(t, decodeChatChunkPayloads(t, buffered))

	finished, err := PatchChatStreamChunk([]byte(`data: {"id":"cmpl_1","object":"chat.completion.chunk","model":"upstream","choices":[{"index":0,"delta":{"content":""},"finish_reason":"tool_calls"}]}`+"\n\n"), "cursor-model", state)
	require.NoError(t, err)

	payloads := decodeChatChunkPayloads(t, finished)
	require.Len(t, payloads, 2)
	toolCalls := payloadChoiceDelta(t, payloads[0])["tool_calls"].([]any)
	function := toolCalls[0].(map[string]any)["function"].(map[string]any)
	require.Equal(t, patch, function["arguments"])
	require.NotContains(t, function["arguments"], `"patch"`)
	require.Equal(t, "tool_calls", payloadChoice(t, payloads[1])["finish_reason"])
}
