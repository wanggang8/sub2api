...
var imageFormatMediaTypes = map[int]string{
	1: "image/png",
	2: "image/jpeg",
	3: "image/gif",
	4: "image/webp",
...
func preprocessRequestBody(raw []byte) ([]byte, error) {
	...
	reconstructed, err := reconstructFromPlaintext(prepared)
	if err != nil {
...
func mergeNormalizedAugmentJSONValue(existing interface{}, incoming interface{}) interface{} {
...
func parseAESKeyIV(raw []byte) ([]byte, []byte, error) {
	...
	if len(parts) != 2 {
		return nil, nil, fmt.Errorf("unexpected decrypted blob length %d and not key::iv format", len(raw))
	}
	hasher := sha256.New()
...
func decodeHexOrBase64(s string) ([]byte, error) {
	...
	out, err := base64.StdEncoding.DecodeString(strings.TrimSpace(s))
	if err == nil {
		return out, nil
	}
	...
	for _, node := range nodes {
		if node.Type == 1 {
			if node.ToolResultNode != nil {
				content := buildToolResultContent(node.ToolResultNode)
				blocks = append(blocks, apicompat.AnthropicContentBlock{
					Type:      "tool_result",
					ToolUseID: effectiveToolUseID(node.ToolResultNode),
					Content:   content,
	...
	for _, node := range nodes {
		...
		case 8:
			if node.Thinking != nil {
				...
				if text != "" {
					blocks = append(blocks, apicompat.AnthropicContentBlock{Type: "thinking", Thinking: text, Signature: strings.TrimSpace(node.Thinking.Signature)})
				}
			}
		}
	}
...
func buildTools(defs []ToolDefinition) []apicompat.AnthropicTool {
	...
	for _, def := range defs {
		...
		schema := def.EffectiveInputSchema()
		tools = append(tools, apicompat.AnthropicTool{
			Name:        name,
			Description: def.Description,
			InputSchema: mustMarshal(schema),
		})
	}
	return tools
}
...
func buildSystem(req *Request) []apicompat.AnthropicContentBlock {
	...
	for _, text := range []string{
		strings.TrimSpace(req.UserGuidelines),
		strings.TrimSpace(req.WorkspaceGuidelines),
		strings.TrimSpace(coerceRulesText(req.Rules)),
		strings.TrimSpace(req.AgentMemories),
		strings.TrimSpace(req.ByokSystemPrompt),
	} {
		if text != "" {
			parts = append(parts, text)
		}
...
func buildThinking(req *Request) *apicompat.AnthropicThinking {
	if thinkingMap, ok := req.Thinking.(map[string]any); ok {
		if typ := strings.TrimSpace(stringValue(thinkingMap["type"])); typ != "" {
			budget := intValue(thinkingMap["budget_tokens"])
			return &apicompat.AnthropicThinking{Type: typ, BudgetTokens: budget}
		}
	}
...
func renderHistorySummaryExchange(entry ChatHistoryEntry) string {
	...
	if len(thinkingParts) > 0 || len(responseParts) > 0 {
		lines = append(lines, "  <agent_response_or_tool_uses>")
		if len(thinkingParts) > 0 {
			lines = append(lines, "    <thinking>")
			lines = append(lines, strings.Join(thinkingParts, "\n"))
			lines = append(lines, "    </thinking>")
		}
...
func formatEditEventsPrompt(node *EditEventsNode) string {
	...
	if text := strings.TrimSpace(node.Source); text != "" {
	...
	for _, event := range node.EditEvents {
		...
		for _, edit := range event.Edits {
			lines = append(lines, fmt.Sprintf("  - edit: after_line_start=%d before_line_start=%d before=%q after=%q", edit.AfterLineStart, edit.BeforeLineStart, edit.BeforeText, edit.AfterText))
		}
	}
	if len(lines) == 1 {
		return ""
...
func formatCheckpointRefPrompt(node *CheckpointRefNode) string {
	...
	if node.FromTimestamp != 0 || node.ToTimestamp != 0 {
		lines = append(lines, fmt.Sprintf("from_timestamp=%d to_timestamp=%d", node.FromTimestamp, node.ToTimestamp))
	}
	if text := strings.TrimSpace(node.Source); text != "" {
		lines = append(lines, "source="+text)
	}
...
func formatFileNodePrompt(node *FileNode) string {
	...
	if strings.HasPrefix(raw, "data:") {
		if idx := strings.Index(raw, ","); idx >= 0 {
			header := raw[:idx]
			b64 = raw[idx+1:]
			if format == "application/octet-stream" && strings.HasPrefix(header, "data:") {
				if semi := strings.Index(header, ";"); semi > len("data:") {
					format = header[len("data:"):semi]
				}
			}
		}
	}

	approxBytes := (len(b64) * 3) / 4
	if !isTextLikeMediaType(format) {
...
func formatHistorySummaryPrompt(node *HistorySummaryNode) string {
	...
	if text := strings.TrimSpace(node.SummarizationRequestID); text != "" {
		lines = append(lines, "summarization_request_id="+text)
	}
	if node.HistoryBeginningDroppedNumExchanges > 0 {
		lines = append(lines, fmt.Sprintf("history_beginning_dropped_num_exchanges=%d", node.HistoryBeginningDroppedNumExchanges))
	}
	if text := strings.TrimSpace(node.MessageTemplate); text != "" {
		lines = append(lines, "message_template="+text)
	}
	if text := strings.TrimSpace(node.SummaryText); text != "" {
		lines = append(lines, "summary_text="+text)
	}
	if text := strings.TrimSpace(node.HistoryMiddleAbridgedText); text != "" {
		lines = append(lines, "history_middle_abridged_text="+text)
	}
	if len(node.HistoryEnd) > 0 {
		lines = append(lines, fmt.Sprintf("history_end_exchanges=%d", len(node.HistoryEnd)))
	}
	if len(lines) == 1 {
		return ""
	}
	lines = append(lines, "[/HISTORY_SUMMARY]")
	return strings.Join(lines, "\n")
}
...
func buildToolResultContent(node *ToolResultNode) json.RawMessage {
	...
	for _, item := range node.ContentNodes {
		...
		case "image":
			...
			if data != "" {
				blocks = append(blocks, apicompat.AnthropicContentBlock{Type: "image", Source: &apicompat.AnthropicImageSource{Type: "base64", MediaType: mediaType, Data: data}})
			}
		case "text":
...
func toolResultImage(item ToolResultContentNode) (string, string) {
	if item.ImageContent != nil {
		...
		if mediaType == "" {
			if mapped := imageFormatMediaTypes[item.ImageContent.Format]; mapped != "" {
				mediaType = mapped
			} else {
				mediaType = "image/png"
			}
...
func hasTextBlock(blocks []apicompat.AnthropicContentBlock) bool {
...
func buildRawImageBlock(raw string, fallbackMediaType string) *apicompat.AnthropicContentBlock {
	...
	return &apicompat.AnthropicContentBlock{
		Type: "image",
