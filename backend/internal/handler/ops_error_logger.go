...
func acquireOpsCaptureWriter(rw gin.ResponseWriter) *opsCaptureWriter {
	...
	w.debugTruncated = false
	w.debugCapturedBytes = 0
	return w
}
...
func (w *opsCaptureWriter) Write(b []byte) (int, error) {
	if w.debugEnabled && len(b) > 0 {
		w.debugCapturedBytes += len(b)
		if !w.debugTruncated {
			remaining := w.debugLimit - w.debugBuf.Len()
			if remaining > 0 {
				if len(b) <= remaining {
					_, _ = w.debugBuf.Write(b)
				} else {
					_, _ = w.debugBuf.Write(b[:remaining])
					w.debugTruncated = true
				}
			} else {
				w.debugTruncated = true
			}
...
func (w *opsCaptureWriter) WriteString(s string) (int, error) {
	if w.debugEnabled && len(s) > 0 {
		w.debugCapturedBytes += len(s)
		if !w.debugTruncated {
			remaining := w.debugLimit - w.debugBuf.Len()
			if remaining > 0 {
				if len(s) <= remaining {
					_, _ = w.debugBuf.WriteString(s)
				} else {
					_, _ = w.debugBuf.WriteString(s[:remaining])
					w.debugTruncated = true
				}
			} else {
				w.debugTruncated = true
...
func (w *opsCaptureWriter) EnableUsageDebugCapture(limit int) {
	if limit <= 0 {
		return
	}
	w.debugEnabled = true
...
func (w *opsCaptureWriter) UsageDebugBodyBytes() []byte {
	return w.debugBuf.Bytes()
}

func (w *opsCaptureWriter) UsageDebugBodyCapturedBytes() int {
	return w.debugCapturedBytes
}

func (w *opsCaptureWriter) UsageDebugBodyTruncated() bool {
	return w.debugTruncated
}
