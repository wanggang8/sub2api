...
type modelsPreviewResult[T any] struct {
...
func (h *AccountHandler) FetchModelsPreview(c *gin.Context) {
	...
	switch req.Platform {
	case service.PlatformOpenAI:
		var fetched []openai.Model
		fetched, fetchErr = h.fetchOpenAIModels(c.Request.Context(), baseURL, apiKey)
		models = modelsPreviewResult[openai.Model]{Models: fetched, Source: "upstream"}
	case service.PlatformGemini:
		var fetched []geminicli.Model
		fetched, fetchErr = h.fetchGeminiModels(c.Request.Context(), baseURL, apiKey)
		models = modelsPreviewResult[geminicli.Model]{Models: fetched, Source: "upstream"}
	case service.PlatformAnthropic:
		var fetched modelsPreviewResult[claude.Model]
		fetched, fetchErr = h.fetchAnthropicModelsPreview(c.Request.Context(), baseURL, apiKey)
		models = fetched
	default:
...
func (h *AccountHandler) FetchModelsFromUpstream(c *gin.Context) {
	...
	case service.PlatformAnthropic:
		models, fetchErr = h.fetchAnthropicModels(c.Request.Context(), baseURL, apiKey)
	case service.PlatformGemini:
...
func (h *AccountHandler) fetchAnthropicModels(ctx context.Context, baseURL, apiKey string) ([]claude.Model, error) {
	result, err := h.fetchAnthropicModelsPreview(ctx, baseURL, apiKey)
	if err != nil {
		return nil, err
	}
...
func (h *AccountHandler) fetchAnthropicModelsPreview(ctx context.Context, baseURL, apiKey string) (modelsPreviewResult[claude.Model], error) {
	trimmedBaseURL := strings.TrimRight(strings.TrimSpace(baseURL), "/")
	if trimmedBaseURL == "" || strings.Contains(strings.ToLower(trimmedBaseURL), "api.anthropic.com") {
		return modelsPreviewResult[claude.Model]{Models: claude.DefaultModels, Source: "default"}, nil
	}

	req, err := http.NewRequestWithContext(ctx, "GET", normalizeOpenAIModelsURL(trimmedBaseURL), nil)
	if err != nil {
		return modelsPreviewResult[claude.Model]{}, err
	}
	...
	if resp.StatusCode != http.StatusOK {
		return modelsPreviewResult[claude.Model]{}, fmt.Errorf("upstream returned status %d", resp.StatusCode)
	}
	...
	if len(result.Data) == 0 {
		return modelsPreviewResult[claude.Model]{}, fmt.Errorf("upstream returned empty model list")
	}
	return modelsPreviewResult[claude.Model]{Models: result.Data, Source: "upstream"}, nil
}
