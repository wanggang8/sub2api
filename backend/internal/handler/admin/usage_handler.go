...
type UsageHandler struct {
	...
	usageDebugService interface {
		Get(ctx context.Context, requestID string) (*service.UsageDebugDetail, error)
		ExportJSON(ctx context.Context, requestID string) ([]byte, error)
	}
}
...
// GetDebugDetail returns short-lived debug request/response payloads by request_id.
// GET /api/v1/admin/usage/debug/:request_id
func (h *UsageHandler) GetDebugDetail(c *gin.Context) {
	if h.usageDebugService == nil {
		response.Error(c, http.StatusServiceUnavailable, "Usage debug service not available")
		return
	}
	...
	detail, err := h.usageDebugService.Get(c.Request.Context(), requestID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	if detail == nil {
		response.NotFound(c, "Usage debug detail not found or expired")
		return
	}
	response.Success(c, detail)
}

// ExportDebugDetail exports one usage debug record as JSON.
// GET /api/v1/admin/usage/debug/:request_id/export
func (h *UsageHandler) ExportDebugDetail(c *gin.Context) {
	if h.usageDebugService == nil {
		response.Error(c, http.StatusServiceUnavailable, "Usage debug service not available")
		return
	}
	...
	raw, err := h.usageDebugService.ExportJSON(c.Request.Context(), requestID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
	...
	c.Header("Content-Disposition", `attachment; filename="usage-debug-`+requestID+`.json"`)
