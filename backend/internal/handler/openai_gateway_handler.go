...
type OpenAIGatewayHandler struct {
	...
	usageDebugService       usageDebugRecorder
...
func NewOpenAIGatewayHandler(
	...
	billingCacheService *service.BillingCacheService,
	apiKeyService *service.APIKeyService,
	usageDebugService *service.UsageDebugService,
	usageRecordWorkerPool *service.UsageRecordWorkerPool,
	errorPassthroughService *service.ErrorPassthroughService,
	cfg *config.Config,
	...
	return &OpenAIGatewayHandler{
		...
		usageDebugService:       usageDebugService,
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	debugCapture := attachUsageDebugCapture(c)
	...
	for {
		...
		effectiveSessionHash := ensureOpenAIPoolModeSessionHash(sessionHash, account)
		...
		if compatPromptCacheKey := h.gatewayService.ResolveCursorCompatPromptCacheKey(c, forwardBody); compatPromptCacheKey != "" {
			patchedBody := service.InjectOpenAIPromptCacheKeyIfMissing(forwardBody, compatPromptCacheKey)
			if effectiveSessionHash == "" {
				effectiveSessionHash = service.DeriveSessionHashFromSeed(compatPromptCacheKey)
			}
			forwardBody = patchedBody
		}
		accountReleaseFunc, acquired := h.acquireResponsesAccountSlot(c, apiKey.GroupID, effectiveSessionHash, selection, reqStream, &streamStarted, reqLog)
		if !acquired {
			return
		}
		...
		debugRequestID := service.ResolveUsageBillingRequestID(c.Request.Context(), result.RequestID)
		requestHeaders := cloneHeaderOrNil(c.Request.Header)
		responseHeaders := cloneHeaderOrNil(c.Writer.Header())
		responseBody, responseBodyBytes, responseBodyTruncated := usageDebugBodySnapshot(debugCapture)
		upstreamRequestBody, _, _ := service.ExtractUsageDebugUpstreamRequestSnapshot(c)
...
func (h *OpenAIGatewayHandler) handleStreamingAwareError(c *gin.Context, status int, errType, message string, streamStarted bool) {
	if streamStarted {
		flusher, ok := c.Writer.(http.Flusher)
		if ok {
			errorEvent := buildCompatStreamingErrorEventWithDefault(c, errType, message, compatStreamErrorFormatOpenAIEvent)
