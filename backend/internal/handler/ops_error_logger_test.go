...
func TestAttachUsageDebugCapture_DoesNotReplaceWriterOrBreakOuterMiddlewares(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	r.GET("/v1/responses", OpsErrorLoggerMiddleware(nil), func(c *gin.Context) {
		before := c.Writer
		capture := attachUsageDebugCapture(c)
		require.NotNil(t, capture)
		require.Same(t, before, c.Writer, "attachUsageDebugCapture should reuse the existing writer wrapper")
